 ...
 *
 * This file initializes key application-wide systems like the auto-player and
 * audio system. It also sets up a development-only bridge for communication
 * with the Vite agent for automated testing and state inspection.
 *
 * The `App` component itself serves as the main layout container, switching
 * between desktop and mobile views and rendering all global UI overlays and modals.
 */
...
/**
 * The root React component for the Frontier application.
 *
 * This component wraps the entire UI in an `ErrorBoundary`, selects the
 * appropriate layout (`DesktopLayout` or `MobileLayout`) based on the screen
 * size, and renders all global UI elements such as overlays, modals, and toasts.
 *
 * @returns The main application component tree.
 */
 ...
 * If the specified track is already active, this function only updates the volume.
 *
 * @param track The target `AmbianceTrack` to play.
 * @param masterVol The current master volume level (0-1).
 * @param musicVol The current music-specific volume level (0-1).
 */
 ...
 * This is intended for use when a user adjusts a volume slider.
 *
 * @param masterVol The master volume level (0-1).
 * @param musicVol The music-specific volume level (0-1).
 */
 ...
 * This is typically used for game end states like victory or game over.
 */
 ...
 *
 * This function sets up listeners for game state changes (e.g., biome, weather,
 * game phase) to automatically manage ambient music and sound effects. It also
 * handles unlocking the browser's AudioContext on the first user gesture to
 * comply with autoplay policies.
 *
 * It should be called once at application startup in a browser environment.
 * The function is a no-op in non-browser environments (e.g., SSR, Vitest).
 */

 ...
 * - An `ErrorBoundary` to catch rendering errors.
 * - A responsive layout that switches between `DesktopLayout` and `MobileLayout`.
 * - All global overlay components, such as modals, briefings, and end screens.
 * - An `ErrorToast` for displaying non-fatal errors.
 *
 * It also contains development-only logic for a Vite agent bridge, which
 * allows external tools to monitor and control the game state during development.
 *
 ...
 *
 * If the target track is already the active one, this function will only
 * update the volume to the new target level without restarting or crossfading.
 * If a different track is playing, it will be faded out while the new track
 * fades in over `CROSSFADE_MS`.
 *
 * This function is a no-op in non-browser environments.
 *
 * @param track The target `AmbianceTrack` to play.
 * @param masterVol The current master volume setting (0-1).
 * @param musicVol The current music-specific volume setting (0-1).
 */
 ...
 *
 * This function should be called when a volume slider is adjusted, as it
 * changes the volume immediately without triggering a crossfade.
 *
 * This function is a no-op in non-browser environments.
 *
 * @param masterVol The current master volume setting (0-1).
 ...
 *
 * This does not stop the track, allowing it to be resumed at the same position.
 *
 * This function is a no-op in non-browser environments.
 *
...
/**
 * Fades out and stops the currently active ambient track.
 *
 * This is typically used for game-end scenarios (victory or game over)
 * to bring the ambient soundscape to a close. It also stops any
 * track that might be in the middle of fading out.
 *
 * This function is a no-op in non-browser environments.
 */
 ...
 *
 * This function must be called once at application startup, after the Zustand
 * store is ready and in a browser context (it is a no-op on the server).
 *
 * It performs the following setup:
 * - Registers a one-time event listener for the first user gesture (click, keydown)
 *   to unlock the browser's `AudioContext`, satisfying autoplay policies.
 * - Subscribes to game state changes (biome, weather, time of day, game phase)
 *   to automatically switch ambient music tracks.
 * - Subscribes to audio preference changes (volume, mute) to adjust playback.
 * - Subscribes to game events (encounters, waypoint arrivals, game end) to
 *   trigger appropriate sound effects.
 * - Calls `updateAmbiance()` once on initialization to start ambient music
 ...
 * oscillator/envelope profile suited to
